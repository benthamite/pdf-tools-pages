...
The package includes a comprehensive ERT test suite in =pdf-tools-pages-test.el=. Because =pdf-tools= requires a compiled C server that cannot run in headless CI environments, all =pdf-tools= functions are mocked with ~cl-letf~. The tests cover page selection, DWIM toggling, qpdf argument construction, exit code handling, buffer-local isolation, and the extract/delete workflows.
...
Both targets expect =pdf-tools= to be available as a sibling directory (=../pdf-tools/lisp=), which is the case for elpaca-managed installations.
...
A GitHub Actions workflow runs byte-compilation and the full test suite on every push and pull request to the =main= branch. Tests are run against Emacs 27.1, 29.4, and 30.1 to ensure broad compatibility.
